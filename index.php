<?php
	...
	$styles = file_get_contents('style/style.css');
	$matches;
	...
	$dev_config = array();
	if (isset($autoconnect) && $autoconnect) {
		$dev_config['autoconnect'] = true;
	}
	if ($dev_config) {
		$tmp = array();
...
<?=$styles_html?>
		<script>
			var con = { log: function () {}, error: function () {} };
		</script>
<?=$script_html?>
		<script>
...
<?php if (isset($host)) : ?>
				<p>Server: <?=$host?></p>
				<p><label>
					MySQL username<br /><input type="text" id="input-username" autofocus />
				</label></p>
				<p><label>
...
<?php else : ?>
				<p><label>
					MySQL host<br /><input type="text" id="input-host" autofocus />
				</label></p>
				<p><label>
...
<?php endif ?>
		...
		<div class="page" id="page-db" hidden>
			...
			<!-- TODO strukturera här - hitta på bättre id/klassnamn -->
			<!--
				possible classes for #search:
					emptyquery
					current-is-dbs current-is-indb
					current-results other-results
					current-noresults other-noresults
			-->
			<div class="column" id="search" hidden>
				...
				<div class="column-content" tabindex="-1">
					...
					<p id="search-noresults">No matching database or table found</p>
					
					<div id="search-current">
						<p id="search-current-noresults-dbs">No matching databases</p>
						<p id="search-current-noresults-indb">No matching tables in <span class="search-current-db-text"></span></p>
					...
					<div id="search-other">
						<p id="search-other-noresults">No more matches</p>
				...
				<div class="column-content" accesskey="3" tabindex="0">
					<div id="result-query">
						...
						<div id="result-query-buttons">
							<button id="result-query-run" title="Run query [Meta+Enter]">Run</button>
