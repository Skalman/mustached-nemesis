$(function () {
	...
	$("#connect-form").on("submit", function (e) {
		e.preventDefault();
		hide($connect_error);
		var $connect_button = $("#connect-button").prop("disabled", true),
			host = $("#input-host").val(),
			config = {
		...
		mysql.config(config);
		show_dbs(function (err, databases) {
	...
	$result_query_run.on("click", function () {
	...
	function perform_search() {
		function mark(match) {
			...
			for (i = matches.length-1; i >= 0; i--) {
				hay = hay.substring(0, matches[i][0]) +
					"<mark>" + hay.substring(matches[i][0], matches[i][1]) + "</mark>" +
					hay.substring(matches[i][1]);
			}
			return hay;
		}
